...
type Item = {
  ...
  status:string; payout_status?:string|null; route_activation_status?:string|null;
  razorpay_linked_account_id?:string|null; razorpay_transfer_id?:string|null;
...
export default function SuperAdminMarketplacePayouts(){
 ...
 const retry=async(id:number)=>{
  setBusy(id);
  try{
   const r=await fetch(`${API}/marketplace/route/payouts/${id}/retry`,{method:"POST",credentials:"include"});
 ...
 const reverse=async(item:Item)=>{
  ...
  setBusy(item.id);
  try{
   const r=await fetch(`${API}/marketplace/route/payouts/${item.id}/reverse`,{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({amount,reason:"Super Admin marketplace reversal"})});
 ...
 return <div className="sa-payout-page">
  <div className="sa-payout-head"><div><h1><ShieldCheck size={23}/> Super Admin · Marketplace Payouts</h1><p>Platform-wide Route settlements, fees, refunds and seller payout controls.</p></div><button onClick={load}><RefreshCw size={16}/> Refresh</button></div>
