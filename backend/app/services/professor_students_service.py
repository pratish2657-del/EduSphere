...
def get_professor_students(user_id, search=None, course_id=None, semester=None, page=1, limit=12):
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM professor_profiles pp INNER JOIN users u ON u.id=pp.user_id
            WHERE pp.user_id=%s LIMIT 1
        """, (user_id,))
        ...
        base = """
            ...
            INNER JOIN institutions i ON i.id=sp.institution_id
            INNER JOIN programs p ON p.id=sp.program_id
            INNER JOIN sections s ON s.id=sp.section_id AND s.program_id=sp.program_id
            INNER JOIN course_enrollments ce ON ce.student_id=su.id
            INNER JOIN courses c ON c.id=ce.course_id
            INNER JOIN timetables t ON t.course_id=c.id AND t.professor_id=%s
            WHERE sp.institution_id=%s AND su.is_active=TRUE
        """
        ...
        if user_ids:
            ...
            cursor.execute(f"""
                SELECT DISTINCT ce.student_id AS user_id, c.id AS course_id,
                       c.name AS course_name, c.code AS course_code
                FROM course_enrollments ce INNER JOIN courses c ON c.id=ce.course_id
                INNER JOIN timetables t ON t.course_id=c.id AND t.professor_id=%s
                WHERE ce.student_id IN ({marks}) ORDER BY c.name ASC
